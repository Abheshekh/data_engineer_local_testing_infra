...
.PHONY: build run stop delete build-notebook run-notebook setup check-env help
...
setup:
	...
	@printf "\nVolumes base path — s3/, delta/, mongodb/, pyspark/ will be created inside it\n(e.g. $(HOME)/Documents/volumes)\n> "; \
	read val; \
	if [ -n "$$val" ]; then \
		sed -i '' "s|VOLUMES_BASE_PATH=.*|VOLUMES_BASE_PATH=$$val|" .env; \
		mkdir -p "$$val/s3" "$$val/delta" "$$val/mongodb" "$$val/pyspark"; \
...
run: check-env
	...
	mkdir -p "$$NOTEBOOK_WORK_PATH" "$$VOLUMES_BASE_PATH/s3" "$$VOLUMES_BASE_PATH/delta" "$$VOLUMES_BASE_PATH/mongodb" "$$VOLUMES_BASE_PATH/pyspark"
...
stop:
	...
	@echo "Containers stopped. Run 'make run CONTAINER_NAME=<name>' to restart."

build-notebook: check-env
	@echo "Building and starting pyspark-notebook service (no dependencies)..."
	@NOTEBOOK_WORK_PATH=$$(grep '^NOTEBOOK_WORK_PATH=' .env | cut -d= -f2-); \
	VOLUMES_BASE_PATH=$$(grep '^VOLUMES_BASE_PATH=' .env | cut -d= -f2-); \
	mkdir -p "$$NOTEBOOK_WORK_PATH" "$$VOLUMES_BASE_PATH/delta" "$$VOLUMES_BASE_PATH/pyspark"
	CONTAINER_TIMESTAMP=$(CONTAINER_TIMESTAMP) docker compose up -d --build pyspark-notebook
...
run-notebook: check-env
	...
	VOLUMES_BASE_PATH=$$(grep '^VOLUMES_BASE_PATH=' .env | cut -d= -f2-); \
	mkdir -p "$$NOTEBOOK_WORK_PATH" "$$VOLUMES_BASE_PATH/delta" "$$VOLUMES_BASE_PATH/pyspark"
	CONTAINER_TIMESTAMP=$(CONTAINER_TIMESTAMP) docker compose up -d pyspark-notebook
